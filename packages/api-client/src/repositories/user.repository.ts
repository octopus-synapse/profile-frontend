...
export function createUserRepository(client: HttpClient) {
 return {
  ...
  async checkUsername(
   ...
   return client.get<{ username: string; available: boolean }>(
    `${BASE_URL}/username/check?username=${encodeURIComponent(username)}`
   );
  },
  ...
  async updateUsername(
   ...
   return client.patch<{ success: boolean; message: string; username: string }>(
    `${BASE_URL}/username`,
    { username }
   );
  },
