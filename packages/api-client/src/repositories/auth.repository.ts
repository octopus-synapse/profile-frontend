...
export function createAuthRepository(client: HttpClient) {
  return {
    ...
    async refreshToken(refreshToken: string): Promise<RefreshTokenResponse> {
      return client.post<RefreshTokenResponse>(`${BASE_URL}/refresh`, { refreshToken });
    },
     ...
     */
    async requestPasswordReset(data: ResetPasswordDto): Promise<{ success: boolean }> {
      return client.post<{ success: boolean }>(`${BASE_URL}/forgot-password`, data);
    },
    ...
    async changePassword(data: ChangePasswordDto): Promise<{ success: boolean }> {
      return client.post<{ success: boolean }>(`${BASE_URL}/change-password`, data);
    },
    ...
    async verifyEmail(token: string): Promise<{ success: boolean }> {
      return client.post<{ success: boolean }>(`${BASE_URL}/verify-email`, { token });
    },
