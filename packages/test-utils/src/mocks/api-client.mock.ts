...
export function createMockApiClient(options: MockApiClientOptions = {}) {
  ...
  return {
    auth: {
      ...
      refreshToken: mock(() =>
        Promise.resolve(defaultResponses.auth?.refreshToken ?? { accessToken: 'new-token' }),
      ),
    ...
    users: {
      ...
      updateMe: mock(() => Promise.resolve(defaultResponses.users?.updateMe ?? { id: 'user-1' })),
      deleteMe: mock(() => Promise.resolve()),
      checkUsername: mock(() =>
        Promise.resolve(defaultResponses.users?.checkUsername ?? { available: true }),
      ),
      checkEmail: mock(() =>
    ...
    resumes: {
      ...
      create: mock(() => Promise.resolve(defaultResponses.resumes?.create ?? { id: 'resume-1' })),
      update: mock(() => Promise.resolve(defaultResponses.resumes?.update ?? { id: 'resume-1' })),
    ...
    themes: {
      ...
      getById: mock(() =>
        Promise.resolve(defaultResponses.themes?.getById ?? { id: 'theme-1', name: 'Default' }),
      ),
      getSystem: mock(() => Promise.resolve(defaultResponses.themes?.getSystem ?? [])),
      create: mock(() => Promise.resolve(defaultResponses.themes?.create ?? { id: 'theme-1' })),
      update: mock(() => Promise.resolve(defaultResponses.themes?.update ?? { id: 'theme-1' })),
      delete: mock(() => Promise.resolve()),
    ...
    twoFactor: {
      ...
      verify: mock(() =>
        Promise.resolve(defaultResponses.twoFactor?.verify ?? { verified: true, backupCodes: [] }),
      ),
      disable: mock(() => Promise.resolve()),
      regenerateBackupCodes: mock(() =>
        Promise.resolve(defaultResponses.twoFactor?.regenerateBackupCodes ?? { codes: [] }),
      ),
    },
    onboarding: {
      getStatus: mock(() =>
    ...
    skills: {
      ...
      getPopular: mock(() => Promise.resolve(defaultResponses.skills?.getPopular ?? [])),
      getCategories: mock(() => Promise.resolve(defaultResponses.skills?.getCategories ?? [])),
    },
  };
}
