...
export function FieldRow({
  ...
  const updateMeta = (patch: Partial<FieldEntry['meta']>) =>
    update({ meta: { ...field.meta, ...patch } });
    ...
    <div className="border-pf-border-default rounded-lg border p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="rounded px-2 py-0.5 text-xs hover:bg-pf-canvas-subtle disabled:opacity-30"
            title={t('admin.sectionTypes.fields.moveUp')}
          >
            ↑
          </button>
          <button
            ...
            className="rounded px-2 py-0.5 text-xs hover:bg-pf-canvas-subtle disabled:opacity-30"
            title={t('admin.sectionTypes.fields.moveDown')}
          >
            ↓
          </button>
          <button
            ...
            title={t('admin.sectionTypes.fields.remove')}
          >
            ✕
          </button>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="space-y-1">
          ...
          <Input
            placeholder={t('admin.sectionTypes.fields.displayName')}
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="space-y-1">
          ...
          <Select
            value={field.meta.widget ?? 'text'}
            onValueChange={(v) => updateMeta({ widget: v as FieldEntry['meta']['widget'] })}
          >
            <SelectTrigger className="h-8 text-sm">
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="flex items-end gap-4 pb-1">
          <div className="flex items-center gap-1.5">
            ...
            <Label htmlFor={`req-${index}`} className="text-xs">
              {t('admin.sectionTypes.fields.required')}
            </Label>
          </div>
          <div className="flex items-center gap-1.5">
            <Checkbox
              ...
              onCheckedChange={(v) => update({ nullable: v === true || undefined })}
            />
            <Label htmlFor={`null-${index}`} className="text-xs">
              {t('admin.sectionTypes.fields.nullable')}
            </Label>
          </div>
        </div>
      </div>
      ...
      {field.type === 'string' && (
        <div className="grid grid-cols-3 gap-3">
          <div className="space-y-1">
            ...
            <Select
              value={field.meta.format ?? ''}
              onValueChange={(v) =>
                updateMeta({ format: (v || undefined) as FieldEntry['meta']['format'] })
              }
            >
              <SelectTrigger className="h-8 text-sm">
                <SelectValue placeholder={t('admin.sectionTypes.fields.none')} />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-1">
            ...
            <Input
              type="number"
              min={0}
              value={field.meta.minLength ?? ''}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                updateMeta({ minLength: e.target.value ? Number(e.target.value) : undefined })
              }
              className="h-8 text-sm"
            />
          </div>
          <div className="space-y-1">
            ...
            <Input
              ...
              onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                updateMeta({ maxLength: e.target.value ? Number(e.target.value) : undefined })
              }
              className="h-8 text-sm"
            />
          </div>
      ...
      {field.type === 'date' && (
        <div className="flex items-center gap-1.5">
          <Checkbox
            id={`present-${index}`}
            checked={field.meta.allowPresentFlag === true}
            onCheckedChange={(v) => updateMeta({ allowPresentFlag: v === true || undefined })}
          />
          <Label htmlFor={`present-${index}`} className="text-xs">
            {t('admin.sectionTypes.fields.allowPresent')}
          </Label>
        </div>
      )}
    </div>
