...
export function SectionHeading({
	...
	return (
		<div className={`mb-12 ${centered ? "text-center" : ""} ${className}`}>
			<h2
				className={`text-4xl font-black uppercase tracking-tighter md:text-6xl ${theme.text}`}
			>
				{children}
				{accent && <span className={theme.accent}> {accent}</span>}
			</h2>
			{subtitle && (
				<p className={`mt-4 text-lg md:text-xl ${theme.textMuted}`}>
					{subtitle}
				</p>
			)}
		</div>
...
export function SectionLabel({ children, centered = true }: SectionLabelProps) {
	...
	return (
		...
		>
			<span
				className={`h-px w-8 ${isDark ? "bg-white/30" : "bg-cyan-600/50"}`}
			/>
			<span
...
export function StatCard({
	...
	return (
		<SectionCard className="text-center">
			...
			<p
				className={`mb-2 text-4xl font-black tracking-tight md:text-5xl ${valueColorClass}`}
			>
				{value}
			</p>
