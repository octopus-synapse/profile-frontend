...
import { LandingSection, SectionCard, SectionHeading, useSectionTheme } from './landing-section';
...
function MarketGrowthChart() {
  ...
  const devsPath = MARKET_DATA.map(
    (d, i) => `${i === 0 ? 'M' : 'L'} ${xScale(i)} ${yScaleDevs(d.devs * animationProgress)}`,
  ).join(' ');

  const jobsPath = MARKET_DATA.map(
    (d, i) => `${i === 0 ? 'M' : 'L'} ${xScale(i)} ${yScaleJobs(d.jobs * animationProgress)}`,
  ).join(' ');
    ...
    <div ref={chartRef} className="relative mx-auto w-full max-w-4xl">
      ...
      <div className="mt-8 grid grid-cols-3 gap-4">
        <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4 text-center">
          ...
          <p className={`text-xs ${theme.textSubtle}`}>crescimento de devs desde 2015</p>
        </div>
        <div className="rounded-xl border border-cyan-500/20 bg-cyan-500/5 p-4 text-center">
          ...
          <p className={`text-xs ${theme.textSubtle}`}>variação de vagas desde 2015</p>
        </div>
        <div className="rounded-xl border border-yellow-500/20 bg-yellow-500/5 p-4 text-center">
          <p className="text-3xl font-black text-zinc-300">
            {Math.round(currentYear.devs / currentYear.jobs)}:1
          </p>
          <p className={`text-xs ${theme.textSubtle}`}>candidatos por vaga em 2025</p>
        </div>
      </div>
    </div>
  );
