...
/**
 * ConsentModal - Landing Page Consent
 *
 * For landing page (unauthenticated users):
 * - Uses localStorage to track consent for modal UX
 * - Backend consent recording happens AFTER authentication during sign-up
 *
 * This is correct because:
 * - Backend consent endpoints require authentication
 * - Landing page users are not authenticated yet
 * - Real consent is recorded when user signs up
 */
export function ConsentModal({ t, isOpen, onClose, onAccept }: ConsentModalProps) {
  ...
  const handleAccept = useCallback(() => {
    ...
    // Store consent locally for UX (prevents showing modal again)
    // Real consent is recorded on the backend during sign-up
    const consentData: StoredConsent = {
      granted: true,
      timestamp: new Date().toISOString(),
      version: '1.0',
    };
    safeLocalStorage.setItem(CONSENT_STORAGE_KEY, JSON.stringify(consentData));
...
/**
 * Hook to manage consent modal state
 *
 * For landing page (unauthenticated users):
 * - Uses localStorage only to check if consent was given
 * - Backend consent happens during sign-up flow
 */
export function useConsentModal() {
  ...
  /**
   * Check if user has previously accepted consent (localStorage only)
   * This is for UX purposes - to skip showing modal if already accepted
   */
