...
export function UsernameStep() {
 const { username, setUsername, goToNextStep, markStepComplete } =
  useOnboardingStore();
  ...
  <div className="space-y-6">
   ...
   <div className="font-mono text-xs text-zinc-500">
    <span className="text-gray-500">
     <span className="opacity-60">{"//"}</span> Letters, numbers, and
     underscores only (3-30 chars)
    </span>
   </div>
   ...
   <div className="space-y-1 border border-white/10 bg-white/5 p-3">
    ...
    <ul className="space-y-1 font-mono text-xs text-zinc-500">
     <li className="flex items-center gap-2">
      <span
       className={inputValue.length >= MIN_LENGTH ? "text-emerald-500" : ""}
      >
     ...
     <li className="flex items-center gap-2">
      <span
       className={inputValue.length <= MAX_LENGTH ? "text-emerald-500" : ""}
      >
       {inputValue.length <= MAX_LENGTH ? "+" : "-"}
      </span>
     ...
     <li className="flex items-center gap-2">
      <span
       className={
        !inputValue || USERNAME_REGEX.test(inputValue) ? "text-emerald-500" : ""
       }
      >
