...
import { useTechNiches, useSearchAllTechSkills } from "@profile/api-client";
...
export function SkillsSection() {
 ...
 const [formData, setFormData] =
  useState<Partial<CreateSkillPayload>>(emptySkill);
 ...
 const { data: searchResults, isLoading: searchLoading } =
  useSearchAllTechSkills(searchQuery, 15);
 ...
 const handleSave = async () => {
  ...
  try {
   const apiPayload: CreateSkillPayload = {
    ...
    category: payload.category ?? formData.category ?? "Technical Skills",
    ...
    <div className="space-y-5 rounded-xl border border-white/10 bg-white/5 p-6">
     ...
     <div>
      ...
      <input
       ...
       onChange={(e) =>
        setFormData((p) => ({ ...p, level: parseInt(e.target.value) }))
       }
