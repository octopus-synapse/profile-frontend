...
function MatchDetails({ details }: { details: Record<string, unknown> }) {
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs font-medium text-zinc-400">Match Breakdown</p>
...
export function JobMatchTool({ resumeId }: JobMatchToolProps) {
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    if (!trimmed) {
      showToast.warning('Empty input', 'Paste a job description to analyze.');
      return;
    }
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-lg font-semibold text-white">Job Match</h3>
        </div>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          {result && !isPending && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2 rounded-lg border border-cyan-500/10 bg-cyan-500/5 px-3 py-2 text-xs text-cyan-300">
                ...
                Optimize your resume to improve the match score
              </div>
            </div>
          )}
        </div>
