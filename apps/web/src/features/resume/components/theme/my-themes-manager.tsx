...
import { useMyThemes, useDeleteTheme, useSubmitForApproval } from "../../hooks";
...
function EmptyState({ onImport, onCreate }: { onImport: () => void; onCreate: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <div className="flex gap-2">
        <button onClick={onImport} className="hover:bg-muted rounded border px-4 py-2 text-sm">
          Import JSON
        </button>
        <button
          onClick={onCreate}
    ...
    <div className="bg-background/90 absolute inset-0 flex items-center justify-center rounded-lg">
      <div className="p-4 text-center">
        ...
        <div className="flex justify-center gap-2">
          <button onClick={onCancel} className="hover:bg-muted rounded border px-3 py-1 text-sm">
            Cancel
          </button>
          <button
            onClick={onDelete}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div className="bg-background w-full max-w-md rounded-lg p-6 shadow-lg">
        ...
        <div className="flex justify-end gap-2">
          <button onClick={onClose} className="hover:bg-muted rounded border px-4 py-2 text-sm">
            Cancel
          </button>
          <button
